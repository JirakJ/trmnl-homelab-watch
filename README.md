...
Disk space, service health and backup freshness. A standalone, MIT-licensed TRMNL plugin with four layouts and a
...
```sh
git clone https://github.com/JirakJ/trmnl-homelab-watch.git
cd trmnl-homelab-watch
python3 -m venv .venv
.venv/bin/pip install -r requirements.lock -e .
mkdir -p private
chmod 700 private
cp config.example.json private/config.json
.venv/bin/python -m trmnl_homelab_watch --demo
```
...
```sh
.venv/bin/python -m trmnl_homelab_watch --config private/config.json
```

1. Download `trmnl-homelab-watch.zip` from this repository's releases.
...
The installed command `trmnl-homelab-watch` accepts the same options. Without `--push`, only
...
`dist/trmnl-homelab-watch.zip`; it contains only `settings.yml` and the five Liquid templates.
