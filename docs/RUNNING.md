...
`/opt/trmnl-homelab-watch`, including its `.venv`. Keep the code read-only to that user and give
...
```sh
sudo cp deploy/trmnl-homelab-watch.service deploy/trmnl-homelab-watch.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now trmnl-homelab-watch.timer
sudo systemctl start trmnl-homelab-watch.service
sudo systemctl status trmnl-homelab-watch.service
```
